...
_stems = np.array([
    ...
    'Fir', 'Acorn', 'Chestnut', 'Pine', 'Beech', 'Cherry', 'Larch', 'Willow', 'Apple', 'Peach', 'Pear'
    'Oak',
...
def gen_towns(num: int = 1) -> pd.Series:
    """
    ...
    The population is a random number ranging from 200 through 5 millions.
    ...
    It is garuanteed that each town name in the generated Series is unique.
    ...
    idx = pd.Index(list(names), dtype="string")
    pops = _get_population(rand, size=len(names))
    towns = pd.Series(pops, index=idx)

    return towns


def _get_population(rand: np.random.Generator, size: int = 1) -> npt.NDArray[np.int64]:
    """
    Generates random population numbers. A power is chosen randomly
    from and equally distributed across an interval 2.5 to 6.7.
    The, 10 is powered by this power.

    rand:
    A random number generator.

    size (default 1):
    Number of population numbers generated.

    returns:
    Numpy array of *size* integers.
    """
    pows = rand.random(size=size) * 4.2 + 2.5
    return (10 ** pows).astype('int64')


def _gen_single_town_name(rand: np.random.Generator) -> str:
    rolls = rand.random(size=3)
    sentence = []
